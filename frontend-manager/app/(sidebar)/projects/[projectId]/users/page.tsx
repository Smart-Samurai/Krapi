...
import { ProjectUser, ProjectScope, Scope } from '@/lib/krapi-client';
import { ScopeGuard } from '@/components/scope-guard';

const PROJECT_SCOPES = [
  { value: ProjectScope.USERS_READ, label: 'Users Read', description: 'View other users' },
  { value: ProjectScope.USERS_WRITE, label: 'Users Write', description: 'Create/update users' },
  { value: ProjectScope.USERS_DELETE, label: 'Users Delete', description: 'Delete users' },
  { value: ProjectScope.DATA_READ, label: 'Data Read', description: 'View project data' },
  { value: ProjectScope.DATA_WRITE, label: 'Data Write', description: 'Create/update data' },
  { value: ProjectScope.DATA_DELETE, label: 'Data Delete', description: 'Delete data' },
  { value: ProjectScope.FILES_READ, label: 'Files Read', description: 'View/download files' },
  { value: ProjectScope.FILES_WRITE, label: 'Files Write', description: 'Upload files' },
  { value: ProjectScope.FILES_DELETE, label: 'Files Delete', description: 'Delete files' },
  { value: ProjectScope.FUNCTIONS_EXECUTE, label: 'Functions Execute', description: 'Run functions' },
  { value: ProjectScope.EMAIL_SEND, label: 'Email Send', description: 'Send emails' },
];
...
export default function ProjectUsersPage() {
  ...
  const loadUsers = async () => {
    ...
    try {
      setLoading(true);
      const response = await krapi.users.getAll(projectId, { search: searchQuery });
  ...
  const toggleUserStatus = async (user: ProjectUser) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast.success(`User ${response.data.is_active ? 'activated' : 'deactivated'}`);
      }
    } catch (error) {
  ...
  const deleteUser = async (userId: string) => {
    if (!krapi || !projectId || !confirm('Are you sure you want to delete this user?')) return;
                      ...
                      <div key={scope.value} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) {
                              setNewUser({ ...newUser, scopes: [...newUser.scopes, scope.value] });
                            } else {
                              setNewUser({ ...newUser, scopes: newUser.scopes.filter(s => s !== scope.value) });
                            }
                          }}
                        />
                        <Label
                ...
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">{user.scopes.length} scopes</span>
                    </div>
                  </TableCell>
                  <TableCell>
