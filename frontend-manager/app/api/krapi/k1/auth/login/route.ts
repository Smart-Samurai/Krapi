...
/**
 * Login API Route (SDK-compatible path)
 *
 * POST /api/krapi/k1/auth/login - User login (admin or project user)
 *
 ...
 *
 * This route handles login for SDK client mode. The SDK's auth.login() method calls this endpoint.
 * Returns response in format that matches backend: { success: true, data: { user, token, session_token, expires_at, scopes } }
 */

export async function POST(request: NextRequest): Promise<Response> {
  // eslint-disable-next-line no-console
  console.log("[FRONTEND] /api/krapi/k1/auth/login called");
  try {
    const body = await request.json();
    // eslint-disable-next-line no-console
    console.log("[FRONTEND] Login request body:", { username: body.username, hasPassword: !!body.password });
    ...
    // SDK client mode expects response wrapped in { success: true, data: {...} } format
    ...
    // We must match this exact format for SDK client mode to properly unwrap it
    const response = {
      success: true,
      data: {
        user: result.user,
    ...
    // eslint-disable-next-line no-console
    console.log("[FRONTEND] Returning login response:", JSON.stringify(response).substring(0, 200));
