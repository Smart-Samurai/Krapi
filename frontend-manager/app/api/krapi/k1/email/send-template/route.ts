...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${backendUrl}/email/send-template`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.error || "Failed to send email from template" },
    ...
    console.error("Send email from template error:", error);
    return NextResponse.json(
