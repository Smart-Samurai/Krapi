...
export async function runCollectionsUITests(testSuite, page) {
  ...
  async function loginAndGetProject() {
    ...
    await projectsContainer.waitFor({ state: "attached", timeout: 8000 }).catch(() => {
    ...
    if (!foundContent) {
      ...
      const errorAlert = await page.locator('[role="alert"], .alert').first().isVisible().catch(() => false);
      const errorText = errorAlert ? await page.locator('[role="alert"], .alert').first().textContent().catch(() => '') : '';
      
      throw new Error(
        `Projects page did not load content. URL: ${url}, Container exists: ${containerExists}, Skeletons: ${skeletonCount}, Cards: ${cardCount}, Empty: ${emptyCount}, Has error: ${errorAlert}, Error text: ${errorText.substring(0, 100)}`
      );
    }
    ...
    if (cardCount === 0) {
      ...
      const url = page.url();
      throw new Error(
        `No project cards found. URL: ${url}, Page contains 'Projects': ${pageText.includes("Projects")}`
      );
    }
  ...
  await testSuite.test("Collections list/table displays", async () => {
    ...
    // Verify we're on a project detail page (should match /projects/[projectId])
    ...
    console.log(`[TEST] Navigating to collections: ${collectionsUrl}`);
    await page.goto(collectionsUrl);
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(5000); // Wait for collections to load
  ...
  await testSuite.test("All collections load correctly", async () => {
    ...
    const collectionsContainer = await page
      .locator('[data-testid="collections-container"]')
      .first()
      .isVisible()
      .catch(() => false);
    ...
    testSuite.assert(
      collectionsContainer || emptyState || hasCollectionText || hasError,
  ...
  await testSuite.test("Create Collection button works", async () => {
    ...
    await page.goto(page.url().replace(/\/$/, "") + "/collections");
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(5000); // Wait for page to load

    const createButton = page
      .locator('[data-testid="create-collection-button"]')
      .first();
    ...
    if (buttonVisible) {
      ...
      const hasModal = await page
        .locator('[data-testid="create-collection-dialog"], [role="dialog"]')
        .first()
        .isVisible()
        .catch(() => false);
  ...
  await testSuite.test("Create Collection form fields display", async () => {
    ...
    if (createButton !== null) {
      ...
      testSuite.assert(nameField, "Collection name field should be visible");
    } else {
  ...
  await testSuite.test(
    ...
    async () => {
      ...
      const collectionItems = await page
        .locator(
          '[class*="collection"], [data-testid*="collection"], tr, [role="listitem"]'
        )
        .all();
      ...
      if (collectionItems.length > 0) {
        ...
        testSuite.assert(
          true,
          "No collections found (empty state should be handled)"
        );
      }
    }
  );
