    ...
    return this.describe("🎯 Projects Management", async () => {
      ...
      await this.test("Create New Project", async () => {
        ...
        this.assertExists(
          response.data,
          "created_at",
          "Project should have created_at"
        );
        this.assertEqual(
          response.data.name,
          projectData.name,
      ...
      await this.test("Get Project by ID", async () => {
        ...
        const response = await axios.get(
          `${CONFIG.BACKEND_URL}/projects/${this.testProject.id}`,
          {
            headers: {
              Authorization: `Bearer ${this.sessionToken}`,
            },
      ...
      await this.test("Get All Projects", async () => {
        ...
        const ourProject = response.data.data.find(
          (p) => p.id === this.testProject.id
        );
        this.assertNotNull(
          ourProject,
      ...
      await this.test("Get Projects with Pagination", async () => {
        const response = await axios.get(
          `${CONFIG.BACKEND_URL}/projects?limit=1&offset=0`,
          {
            headers: {
              Authorization: `Bearer ${this.sessionToken}`,
            },
      ...
      await this.test("Create Multiple Projects for Testing", async () => {
        ...
        responses.forEach((response, index) => {
          this.assertHttpSuccess(
            response,
            `Project ${index} creation should succeed`
          );
          this.createdProjects.push(response.data.id);
        });
      ...
      await this.test("Get Project Settings", async () => {
        const response = await axios.get(
          `${CONFIG.BACKEND_URL}/projects/${this.testProject.id}/settings`,
          {
            headers: {
              Authorization: `Bearer ${this.sessionToken}`,
            },
      ...
      await this.test("Update Project Settings", async () => {
        ...
        this.assertHttpSuccess(
          response,
          "Update project settings should succeed"
        );
        this.assertEqual(
      ...
      await this.test("Get Project Statistics", async () => {
        const response = await axios.get(
          `${CONFIG.BACKEND_URL}/projects/${this.testProject.id}/statistics`,
          {
            headers: {
              Authorization: `Bearer ${this.sessionToken}`,
            },
        ...
        this.assertHttpSuccess(
          response,
          "Get project statistics should succeed"
        );
        this.assertExists(
          response.data,
          "totalCollections",
          "Stats should include collection count"
        );
        this.assertExists(
          response.data,
          "totalDocuments",
        ...
        this.assertExists(
          response.data,
          "totalUsers",
          "Stats should include user count"
        );
        this.assertType(
          response.data.totalCollections,
          "number",
          "Collection count should be a number"
        );
        this.assertType(
          response.data.totalDocuments,
          "number",
          "Document count should be a number"
        );
        this.assertType(
      ...
      await this.test("Get Project Activity", async () => {
        const response = await axios.get(
          `${CONFIG.BACKEND_URL}/projects/${this.testProject.id}/activity`,
          {
            headers: {
              Authorization: `Bearer ${this.sessionToken}`,
            },
      ...
      await this.test("Get Project Activity with Filters", async () => {
        const response = await axios.get(
          `${CONFIG.BACKEND_URL}/projects/${this.testProject.id}/activity?limit=5&days=1`,
          {
            headers: {
              Authorization: `Bearer ${this.sessionToken}`,
            },
        ...
        this.assertHttpSuccess(
          response,
          "Filtered activity request should succeed"
        );
        this.assertTrue(
          response.data.activities.length <= 5,
          "Should respect limit parameter"
        );
      });
      ...
      await this.test("Invalid Project Operations", async () => {
        // Test get non-existent project with valid UUID format
        try {
          await axios.get(
            `${CONFIG.BACKEND_URL}/projects/00000000-0000-0000-0000-000000000000`,
            {
              headers: {
                Authorization: `Bearer ${this.sessionToken}`,
              },
      ...
      await this.test("Project Search and Filtering", async () => {
        // Test ordering
        const orderedResponse = await axios.get(
          `${CONFIG.BACKEND_URL}/projects?orderBy=created_at&order=desc`,
          {
            headers: {
              Authorization: `Bearer ${this.sessionToken}`,
            },
        ...
        this.assertHttpSuccess(
          orderedResponse,
          "Ordered request should succeed"
        );
        this.assertTrue(
          orderedResponse.data.data.length > 0,
        ...
        if (orderedResponse.data.data.length > 1) {
          ...
          this.assertTrue(
            first >= second,
            "Projects should be ordered by creation date desc"
          );
        }
      });
    });
