...
class AuthTests extends TestFramework {
  ...
  async runAll() {
    return this.describe("🔐 Authentication & Session Management", async () => {
      // Test 1: Admin Login - through frontend API route
      await this.test("Admin Login with Default Credentials", async () => {
        ...
        this.assertExists(
          response.data,
          "expires_at",
          "Response should contain expiration"
        );
        this.assertType(
      ...
      await this.test("Get Current User with Session Token", async () => {
        this.assertNotNull(
          this.sessionToken,
          "Session token should be available"
        );
      ...
      await this.test("Validate Session Token", async () => {
        const response = await axios.post(
          `${CONFIG.FRONTEND_URL}/api/auth/validate`,
          {
        ...
        this.assertExists(
          response.data,
          "user",
          "Validation should return user data"
        );
      });
      ...
      await this.test("Refresh Session Token", async () => {
        const response = await axios.post(
          `${CONFIG.FRONTEND_URL}/api/auth/refresh`,
          {},
          {
            headers: {
              Authorization: `Bearer ${this.sessionToken}`,
        ...
        this.assertExists(
          response.data,
          "session_token",
          "Refresh should return new session token"
        );
        this.assertExists(
          response.data,
          "expires_at",
          "Refresh should return new expiration"
        );
      ...
      await this.test("Create API Key Session", async () => {
        ...
        const createKeyResponse = await axios.post(
          `${CONFIG.FRONTEND_URL}/api/admin/api-keys`,
          {
            name: "Test API Key",
            permissions: [
              "admin:read",
        ...
        if (createKeyResponse.status === 201) {
          ...
          const sessionResponse = await axios.post(
            `${CONFIG.FRONTEND_URL}/api/auth/sessions`,
            {
          ...
          this.assertHttpSuccess(
            sessionResponse,
            "API key session creation should succeed"
          );
          this.assertExists(
          ...
          this.assertExists(
            sessionResponse.data,
            "expires_at",
            "Should return expiration"
          );
          this.assertExists(
            sessionResponse.data,
            "scopes",
            "Should return scopes"
          );
        } else {
          // Skip this test if we can't create API keys yet
          console.log(
            "      Skipping API key test - admin API keys not yet implemented"
          );
        }
      });
      ...
      await this.test("Change Admin Password", async () => {
        ...
        const response = await axios.post(
          `${CONFIG.FRONTEND_URL}/api/auth/change-password`,
          {
        ...
        // Update the config password so other tests use the new password
      ...
      await this.test("Invalid Login Attempts", async () => {
        ...
        try {
          ...
          this.assertEqual(
            error.response.status,
            401,
            "Should return 401 for wrong username"
          );
        }
      });
      ...
      await this.test("Logout and Invalidate Session", async () => {
        const response = await axios.post(
          `${CONFIG.FRONTEND_URL}/api/auth/logout`,
          {},
          {
            headers: {
              Authorization: `Bearer ${this.sessionToken}`,
        ...
        try {
          ...
          this.assertEqual(
            error.response.status,
            401,
            "Should return 401 for invalidated token"
          );
        }
      ...
      await this.test("Session Persistence and Reuse", async () => {
        ...
        for (let i = 0; i < 3; i++) {
          const response = await axios.get(
            `${CONFIG.FRONTEND_URL}/api/auth/me`,
            {
              headers: {
                Authorization: `Bearer ${this.sessionToken}`,
              },
