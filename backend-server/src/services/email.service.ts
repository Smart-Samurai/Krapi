...
export class EmailService {
  ...
  private getTransporter(project: Project): nodemailer.Transporter | null {
    ...
    try {
      ...
      console.error(`Failed to create email transporter for project ${project.id}:`, error);
      return null;
    }
  }
  ...
  async sendEmail(
    ...
    try {
      ...
      if (!transporter) {
        return { success: false, error: 'Email not configured for this project' };
      }

      // Get email config
      const emailConfig = project.settings.email_config || this.getDefaultConfig();
      if (!emailConfig) {
  ...
  async testEmailConfig(emailConfig: EmailConfig): Promise<{ success: boolean; error?: string }> {
    try {
      const transporter = nodemailer.createTransport({
        host: emailConfig.smtp_host,
        port: emailConfig.smtp_port,
