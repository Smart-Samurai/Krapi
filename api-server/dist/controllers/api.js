...
class ApiController {
    ...
    static async toggleApiKeyStatus(req, res) {
        try {
            const { id } = req.params;
    ...
    // Create API endpoint
    static async createApiEndpoint(req, res) {
        try {
            const { method, path, handler, description, auth_required, permissions, rate_limit, } = req.body;
    ...
    // Update API endpoint
    static async updateApiEndpoint(req, res) {
        try {
            const { id } = req.params;
            const updates = req.body;
    ...
    // Delete API endpoint
    static async deleteApiEndpoint(req, res) {
        try {
            const { id } = req.params;
    ...
    // Create rate limit
    static async createRateLimit(req, res) {
        try {
            const { name, requests_per_minute, requests_per_hour, requests_per_day, applies_to, } = req.body;
    ...
    // Delete rate limit
    static async deleteRateLimit(req, res) {
        try {
            const { id } = req.params;
...
ApiController.db = database_1.default;
