...
export class HealthHttpClient extends BaseHttpClient {
  ...
  async checkStorage(): Promise<ApiResponse<{
    healthy: boolean;
    message: string;
  ...
  async checkEmail(): Promise<ApiResponse<{
    healthy: boolean;
    message: string;
    details?: Record<string, unknown>;
  }>> {
  ...
  async checkAll(): Promise<ApiResponse<{
    overall: boolean;
    services: {
  ...
  // Diagnostics
  async runDiagnostics(): Promise<ApiResponse<{
    summary: {
      total: number;
      passed: number;
      failed: number;
  ...
  async runDatabaseDiagnostics(): Promise<ApiResponse<{
    summary: {
      total: number;
      passed: number;
      failed: number;
  ...
  async runStorageDiagnostics(): Promise<ApiResponse<{
    summary: {
      total: number;
      passed: number;
      failed: number;
  ...
  async runEmailDiagnostics(): Promise<ApiResponse<{
    summary: {
      total: number;
      passed: number;
      failed: number;
  ...
  async validateProjectSchema(projectId: string): Promise<ApiResponse<SchemaValidationResult>> {
    return this.get<SchemaValidationResult>(`/projects/${projectId}/health/schema`);
  }
  ...
  async validateCollectionSchema(
    ...
    return this.get<SchemaValidationResult>(`/projects/${projectId}/collections/${collectionId}/health/schema`);
  }

  // Auto-Fix
  async autoFix(): Promise<ApiResponse<AutoFixResult>> {
    return this.post<AutoFixResult>("/health/auto-fix");
  }
  ...
  async autoFixCollection(
    ...
    return this.post<AutoFixResult>(`/projects/${projectId}/collections/${collectionId}/health/auto-fix`);
  }

  // Database Operations
  async repairDatabase(): Promise<ApiResponse<{
    success: boolean;
    message: string;
  ...
  // Migrations
  async getMigrations(): Promise<ApiResponse<{
    applied: Array<{
      id: string;
      name: string;
      applied_at: string;
